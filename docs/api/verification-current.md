...
4. S3 PUT 성공 후 `POST /api/v1/me/groups/{groupId}/verifications/current/submit`

Android가 PUT 성공을 주장하더라도 Backend는 bound private object를 `HeadObject`로 다시 확인한다. Spring Backend는 media bytes를 relay하지 않는다.
...
응답은 `Cache-Control: no-store`다. `requiredHeaders`의 모든 값을 S3 PUT에 그대로 적용해야 한다. `uploadUrl`은 임시 credential이므로 Android와 Backend 모두 전체 URL을 log에 남기지 않는다. Object key와 bucket은 public contract가 아니다.
...
이미 제출된 Verification은 `200 OK`로 현재 persisted status와 최초 `submittedAt`을 반환한다. S3 PUT 실패 시 submit을 호출하지 않는 것이 정상 UX이며, 호출하더라도 Backend는 `409 Conflict`로 거부한다.
...
오류 응답 body는 비어 있으며 내부 ID, object key, bucket, 예외 message를 반환하지 않는다.
